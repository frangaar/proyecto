...
preguntas = [
    ...
    ['5) Realiza esta operación: <b>24.498,21</b> + <b>31.754,1</b> + <b>66.151</b> - <b>804</b> (Poner los decimales con un punto)',121599.31]
    ];
...
for (let index = 0; index < preguntas.length; index++) {
    ...
    html += '<p>'+preguntas[index][0]+'</p>';
    html += '</div>';
    html += '<div class="col-3">';
    html += '<input class="form-control form-control-sm respuestas preguntas" placeholder="Escribe tu respuesta" type="number">';
    html += '</div>';
    html += '</div>';
...
function comprobarRespuestas(){
    ...
    for (let index = 0; index < respuestas.length; index++) {
        ...
        if(respuestas[index].value == preguntas[index][1]){
            aciertos++;
        }
    }
